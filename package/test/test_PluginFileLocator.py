...
class PluginFileAnalyzerWithInfoFileTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.version_plugin_path = os.path.join(
            self.plugin_directory, "versioned11.version-plugin"
        )
        self.yapsy_filter_plugin_path = os.path.join(
            self.plugin_directory, "simpleplugin.yapsy-filter-plugin"
        )
    ...
    def test_getInfosDictFromPlugin(self):
        ...
        info_dict, cf_parser = analyzer.getInfosDictFromPlugin(
            self.plugin_directory, os.path.basename(self.yapsy_plugin_path)
        )
        self.assertEqual(
            info_dict,
            {
    ...
    def test__extractCorePluginInfo_with_str_filename(self):
        ...
        infos, parser = analyzer._extractCorePluginInfo(
            self.plugin_directory, plugin_desc_content
        )
...
class PluginFileAnalyzerMathingRegexTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.version_plugin_10_path = os.path.join(
            self.plugin_directory, "VersionedPlugin10.py"
        )
        self.version_plugin_12_path = os.path.join(
            self.plugin_directory, "VersionedPlugin12.py"
        )
    ...
    def test_getInfosDictFromPlugin(self):
        analyzer = PluginFileAnalyzerMathingRegex("mouf", r".*VersionedPlugin\d+\.py$")
        info_dict, cf_parser = analyzer.getInfosDictFromPlugin(
            self.plugin_directory, os.path.basename(self.version_plugin_10_path)
        )
        self.assertEqual(
            info_dict,
...
class PluginFileLocatorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.plugin_as_dir_directory = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "pluginsasdirs"
        )
    ...
    def test_locatePlugins(self):
        ...
        self.assertEqual(
            os.path.join(self.plugin_directory, self.plugin_info_file), candidates[0][0]
        )
        self.assertEqual(
            os.path.join(self.plugin_directory, self.plugin_name), candidates[0][1]
        )
    ...
    def test_locatePlugins_when_plugin_is_symlinked(self):
        ...
        try:
            ...
            os.symlink(
                os.path.join(self.plugin_directory, plugin_info_file),
                os.path.join(temp_dir, plugin_info_file),
            )
            os.symlink(
                os.path.join(self.plugin_directory, plugin_impl_file),
            ...
            self.assertEqual(len(candidates), num)
            self.assertEqual(
                os.path.join(temp_dir, self.plugin_info_file), candidates[0][0]
            )
            self.assertEqual(os.path.join(temp_dir, self.plugin_name), candidates[0][1])
            self.assertTrue(isinstance(candidates[0][2], PluginInfo))
        finally:
    ...
    def test_locatePlugins_when_plugin_is_a_directory(self):
        ...
        self.assertEqual(len(candidates), num)
        self.assertEqual(
            os.path.join(self.plugin_as_dir_directory, self.plugin_info_file),
            candidates[0][0],
        )
        self.assertEqual(
            os.path.join(self.plugin_as_dir_directory, self.plugin_name, "__init__"),
            candidates[0][1],
        )
        self.assertTrue(isinstance(candidates[0][2], PluginInfo))
    ...
    def test_locatePlugins_when_plugin_is_a_symlinked_directory(self):
        ...
        try:
            ...
            os.symlink(
                os.path.join(self.plugin_as_dir_directory, plugin_info_file),
                os.path.join(temp_dir, plugin_info_file),
            )
            os.symlink(
                os.path.join(self.plugin_as_dir_directory, plugin_impl_dir),
            ...
            self.assertEqual(
                os.path.join(temp_dir, self.plugin_name, "__init__"), candidates[0][1]
            )
            self.assertTrue(isinstance(candidates[0][2], PluginInfo))
        finally:
            shutil.rmtree(temp_dir)
    ...
    def test_locatePlugins_recursively_when_plugin_is_a_directory(self):
        ...
        try:
            ...
            self.assertEqual(len(candidates), num)
            self.assertEqual(
                os.path.join(temp_sub_dir, self.plugin_info_file), candidates[0][0]
            )
            self.assertEqual(
...
class PluginManagerSetUpTest(unittest.TestCase):
    ...
    def test_setPluginLocator_with_invalid_locator(self):
        ...
        pm = PluginManager()
        self.assertRaises(TypeError, pm.setPluginLocator, SpecificLocator())
