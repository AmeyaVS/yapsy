...
class YapsyUtils(unittest.TestCase):
    ...
    def test_NormalizePluginNameForModuleName_on_name_with_nonalphanum(self):
        self.assertEqual(
            "mouf__glop_a_é", NormalizePluginNameForModuleName("mouf+?glop:a/é")
        )
...
class SimplePluginAdvancedManipulationTestsCase(unittest.TestCase):
    ...
    def testTwoStepsLoad(self):
        ...
        loadedPlugins = spm.loadPlugins(
            callback=preload_cbk, callback_after=postload_cbk
        )
    ...
    def testGetPluginOf(self):
        ...
        self.assertEqual(
            len(
                ...
                | spm.getPluginsOf(categories=["Default", "IP"])
            ),
            1,
        )
        self.assertEqual(
            len(spm.getPluginsOf(category="Default") | spm.getPluginsOf(category="IP")),
            1,
        )
...
class SimplePluginDetectionTestsCase(unittest.TestCase):
    ...
    def testDisablingRecursivePluginLocationIsEnforced(self):
        ...
        pluginLocator.setPluginPlaces([os.path.dirname(os.path.abspath(__file__))])
    ...
    def testEnforcingPluginDirsDoesNotKeepDefaultDir(self):
        ...
        self.assertFalse(
            set(candidates_files_in_default_dir).issubset(set(candidates_files))
        )
...
suite = unittest.TestSuite(
    [
        ...
        unittest.TestLoader().loadTestsFromTestCase(
            SimplePluginAdvancedManipulationTestsCase
        ),
