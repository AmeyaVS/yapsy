...
class AutoInstallTestsCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            os.remove(
                os.path.join(
                    self.pluginManager.plugins_places[0], "AutoInstallPlugin.py"
                )
            )
        except OSError:
            pass
        ...
        try:
            shutil.rmtree(
                os.path.join(
                    self.pluginManager.plugins_places[0], "autoinstalldirplugin"
                )
            )
        except OSError:
            pass
    ...
    def testInstallFile(self):
        ...
        install_success = self.pluginManager.install(
            self.new_plugins_waiting_dir, "autoinstallplugin.yapsy-autoinstall-plugin"
        )
...
class AutoInstallZIPTestsCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            shutil.rmtree(
                os.path.join(
                    self.pluginManager.plugins_places[0], "autoinstallzipplugin"
                )
            )
        except OSError:
            pass
    ...
    def testInstallZIP(self):
        ...
        test_file = os.path.join(
            self.new_plugins_waiting_dir, "autoinstallZIPplugin.zip"
        )
        if sys.version_info < (2, 6):
            self.assertRaises(
                NotImplementedError, self.pluginManager.installFromZIP, test_file
            )
            return
    ...
    def testInstallZIPFailOnWrongZip(self):
        ...
        """
        test_file = os.path.join(
            self.new_plugins_waiting_dir, "autoinstallWRONGzipplugin.zip"
        )
        if sys.version_info < (2, 6):
            self.assertRaises(
                NotImplementedError, self.pluginManager.installFromZIP, test_file
