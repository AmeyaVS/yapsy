...
class AutoInstallPluginManager(PluginManagerDecorator):
    ...
    def __init__(
        ...
        PluginManagerDecorator.__init__(
            self,
            decorated_manager,
            categories_filter,
            directories_list,
    ...
    def install(self, directory, plugin_info_filename):
        ...
        # directory or a file
        if not (
            os.path.exists(plugin_info.path) or os.path.exists(plugin_info.path + ".py")
        ):
            log.warning(
                "Could not find the plugin's implementation for %s." % plugin_info.name
            )
            return False
        if os.path.isdir(plugin_info.path):
            try:
                ...
                shutil.copy(
                    os.path.join(directory, plugin_info_filename), self.install_dir
                )
            except:
    ...
    def installFromZIP(self, plugin_ZIP_filename):
        ...
        if not os.path.isfile(plugin_ZIP_filename):
            log.warning(
                "Could not find the plugin's zip file at '%s'." % plugin_ZIP_filename
            )
            return False
        try:
            ...
            if first_bad_file:
                raise Exception(
                    "Corrupted ZIP with first bad file '%s'" % first_bad_file
                )
        ...
        log.info(
            "Investigating the content of a zip file containing: '%s'" % zipContent
        )
        log.info(
        ...
        isValid = False
        log.info(
            "Looking for the zipped plugin's info file among '%s'" % infoFileCandidates
        )
