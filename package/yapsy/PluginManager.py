...
class PluginManager(object):
    ...
    def _locatorDecide(self, plugin_info_ext, plugin_locator):
        ...
        *plugin_locator*.
        ...
        if not specific_info_ext and not specific_locator:
            ...
            # will be ignored
            msg = (
                "Two incompatible arguments (%s) provided:",
    ...
    def _getPluginNameAndModuleFromStream(
        self, infoFileObject, candidate_infofile="<buffered info>"
    ):
        """
        DEPRECATED(>1.9): please use a specific plugin
        locator if you need such information.
        ...
        return self.getPluginLocator().getPluginNameAndModuleFromStream(
            infoFileObject, candidate_infofile
        )
        ...
        for candidate_infofile, candidate_filepath, plugin_info in self._candidates:
            ...
            for element, element_name in (
                ...
                for category_name in self.categories_interfaces:
                    ...
                    if (
                        is_correct_subclass
                        and element is not self.categories_interfaces[category_name]
                    ):
                        current_category = category_name
                        if (
                            candidate_infofile
                            not in self._category_file_mapping[current_category]
                        ):
                            # we found a new plugin: initialise it and search for the next one
                            if not plugin_info_reference:
                                try:
                                    plugin_info.plugin_object = (
                                        self.instanciateElementWithImportInfo(
                                            element,
                                            element_name,
                                            plugin_module_name,
                                            candidate_filepath,
                                    ...
                                    exc_info = sys.exc_info()
                                    last_failed_attempt_message = (
                                        "Unable to create plugin object: %s"
                                        % candidate_filepath
                                    )
                                    log.debug(
